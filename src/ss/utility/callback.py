...
from typing import DefaultDict, Self, cast
...
class Callback:
    ...
    def __getitem__(self, key: str) -> NDArray[np.float64]:
        ...
        assert key in self._callback_params.keys(), (
    ...
    def add_meta_data(
        ...
        **meta_data_dict: ArrayLike | MetaData,
    ) -> None:
        """
    ...
    def _save_callback_params(
        ...
        if "/" in key:
            ...
            if isinstance(h5_subgroup, h5py.Group):
                ...
                # If the parent key already exists as a dataset,
                # convert it to a dataset under the parent key group
                # with the same name as the parent key.
