...
class PerformanceCallback(Callback, Generic[P]):
    ...
    def _record(self, time: float) -> None:
        super()._record(time)
        self._callback_params["utilization"].append(
            self._performance.utilization
        )
        self._callback_params["temperature"].append(
            self._performance.temperature
        )
        self._callback_params["total_memory"].append(
            self._performance.total_memory
        )
        self._callback_params["free_memory"].append(
            self._performance.free_memory
        )
        self._callback_params["used_memory"].append(
            self._performance.used_memory
        )
        self._callback_params["memory_percent"].append(
