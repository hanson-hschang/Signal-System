...
class TestHmmFilter:
    ...
    def test_filtering_from_simulate_batch(self) -> None:
        ...
        result = simulate(system, 0.0, 6, init_states, scan_key)
        assert result.states.shape == (7, 4, 1)
        assert result.observations.shape == (6, 4, 1)

        initial_belief = jnp.full((4, 2), 0.5)
        filter_times, beliefs = filtering(hmm_filter, 0.0, initial_belief, result.observations)
        assert jnp.allclose(filter_times, result.times[1:])
