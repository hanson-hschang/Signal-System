...
class TestDiscreteTimeLinearSystem:
    ...
    def test_initialization(self):
        assert np.all(self.control_system.state_space_matrix_A == self.state_space_matrix_A)
        assert np.all(self.control_system.state_space_matrix_B == self.state_space_matrix_B)
        assert np.all(self.control_system.state_space_matrix_C == self.state_space_matrix_C)
        assert np.all(
        ...
        assert np.all(
            ...
            == np.zeros(
                (self.state_space_matrix_C.shape[0], self.state_space_matrix_C.shape[0])
            )
        )
