...
class TestMassSpringDamperSystem:
    def test_builds_expected_continuous_dynamics(self) -> None:
    ...
    def test_observation_and_control_choices_select_physical_coordinates(self) -> None:
    ...
    def test_exact_discrete_step_matches_exposed_state_space_matrices(self) -> None:
        ...
        expected = system.discrete_state_matrix @ state + system.discrete_control_matrix @ control
        assert jnp.allclose(next_time, system.time_step)
        assert jnp.allclose(next_state, expected)

    def test_initialization_and_duplication_are_batched(self) -> None:
        system = MassSpringDamperSystem(
        ...
        assert duplicate.initial_state().shape == (5, 6)
        assert duplicate.discrete_state_matrix.shape == (6, 6)
        assert system.batch_size == 2

    def test_noise_covariances_are_discrete_and_used_without_time_scaling(self) -> None:
        ...
        _, process_samples = jax.vmap(lambda key: system.process(0.0, zero_state, None, key))(process_keys)
        observation_samples = jax.vmap(lambda key: system.observe(0.0, zero_state, key))(observation_keys)
