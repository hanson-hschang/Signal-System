...
def plot_simulation(
    ...
    """Plot every batch member of an open-loop mass-spring-damper run."""
    ...
    for batch_index in range(states.shape[1]):
        for connection_index in range(number_of_connections):
            ...
            axes[1].plot(
                times,
                states[:, batch_index, number_of_connections + connection_index],
                linewidth=1.2,
                alpha=0.8,
                label=label,
            )
    ...
    axes[1].set_title("Mass velocities")
    ...
    figure.suptitle("Mass-spring-damper simulation")
...
def render_animation(
    ...
    axis.set_title("Mass-spring-damper simulation")
    ...
    writer = FFMpegWriter(fps=fps, metadata={"title": "Mass-spring-damper simulation"})
