...
def observation_generator(
    ...
    """
    Generate the pair (observation, next_observation) from the
    observation_trajectory over the time_horizon.
...
def cross_entropy(
    ...
    :math: `p^*_{nc}` is the `target_probability`, and
    :math: `p_{nc}` is the `input_probability`.
